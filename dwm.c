...
struct Client { /* a window that dwm is managing */
	...
	int nonotch; /* if 1, this client still gets maximalist floating/border, but never a notch */
	...
	Window dockicon; /* the app's real content window (its icon_window if it set one, else its own window), reparented as a child of c->win; None if not a dockapp */
...
struct Monitor {
	...
	int showbar;
	int premaxshowbar; /* bar visibility before Maximalist Mode forced it hidden, restored on toggle-off */
	int showtitle;
	int showtags;
	int showlayout;
	int showstatus;
...
typedef struct {
	...
	int nomaximalist; /* 1 = this app is never floated/decorated by maximalist mode */
	int nonotch; /* 1 = still gets maximalist floating/border, but never a notch */
...
static unsigned int notchwidth(Client *c);
...
static Client *wintonotch(Window w);
...
static Clr dockbg, dockhi, docklo; /* flat classic-WindowMaker light gray + its hi/lo bevel shades — deliberately NOT pywal-derived yet, see drawdocktile() */
static Display *dpy;
static Drw *drw;
...
{
	/* Any window whose class/instance matches a rule with .isdockapp gets
	 * wrapped in a bezeled background tile — real WindowMaker binaries
	 * (via their XWMHints icon_window) and our own PywalDock-toolkit
	 * apps (which have no icon_window, so we wrap the window itself)
	 * alike. This runs before updatetitle()/Client alloc, so — unlike
	 * applyrules() — it only has class/instance to go on (title-only
	 * rule variants are deliberately skipped here, they still apply
	 * normally later via applyrules()). */
...
configurerequest(XEvent *e)
{
	...
	if ((c = wintoclient(ev->window))) {
		if (ev->value_mask & CWBorderWidth)
			...
			if (ISVISIBLE(c))
				XMoveResizeWindow(dpy, c->win, c->x, c->y, c->w, c->h);
			if (c->isdockapp && (ev->value_mask & (CWWidth|CWHeight)))
				drawdocktile(c); /* repaint bezel to match new size — e.g. an external script wmctl-resizing the tile */
...
createnotch(Client *c)
{
	...
	XSetWindowBorder(dpy, c->twin, scheme[SchemeNotchNorm][ColBorder].pixel);
	if (!c->staticlabel) {
		strncpy(c->notchlabel, c->name, sizeof c->notchlabel - 1);
		c->notchlabel[sizeof c->notchlabel - 1] = '\0';
	}
	updatenotchpos(c); /* applies correct border-aware geometry immediately */
	if (maximalistmode) {
		ensurenotchroom(c);
...
notchwidth(Client *c)
{
	return MAX((int)c->w - 2 * (int)maximalistborderpx, 1);
}
...
{
	XWindowChanges wc;
	unsigned int nbw = maximalistborderpx; /* the notch keeps its own frame even though the client itself is now always borderless */

	if (!c->twin)
		return;
	wc.x = c->x;
	wc.y = c->y - bh - 2 * nbw;
	wc.width = notchwidth(c);
	wc.height = bh;
	wc.border_width = nbw;
	XConfigureWindow(dpy, c->twin, CWX|CWY|CWWidth|CWHeight|CWBorderWidth, &wc);
}
...
ensurenotchroom(Client *c)
{
	...
	miny = c->mon->my + (int)gappov + bh + 2 * maximalistborderpx; /* leave room for the notch (plus its own border) above the window — same topmargin formula movecorner()/resizemaximalist() use, was missing gappov here */
	if (c->y < miny)
...
updatebezelcolors(void)
{
	...
	/* classic Motif/WindowMaker dock gray — flat #AEAAAE base with a
	 * white highlight and a dark gray shadow, independent of the pywal
	 * scheme colors above (dockapp tiles get their own palette hook later) */
	drw_clr_create(drw, &dockbg, "#AEAAAE");
	drw_clr_create(drw, &dockhi, "#FFFFFF");
	drw_clr_create(drw, &docklo, "#555555");
}

/* Paints the tile behind a wrapped dockapp's content window — plain Xlib
 * calls straight onto c->win, not the shared bar `drw` pixmap (which is
 * only bh tall and too small for a ~64px dockapp tile). Flat classic
 * Motif/WindowMaker light gray, not pywal-derived yet (see updatebezelcolors()
 * — a pywal hook for this is planned separately). Single outset bevel —
 * highlight top/left, shadow bottom/right — the classic "raised tile"
 * look real WindowMaker dock tiles have. (Tried a second inverted ring
 * around the icon for extra depth; dropped it — highlight ended up on
 * all four sides at once between the two rings, reading as a solid
 * white outline instead of a 3D edge.) Dock tiles are always drawn in
 * the unfocused palette: dockapp rules carry .nofocus, so they're never
 * "selected" in the focus sense. Called both right after the tile is
 * first mapped and again any time it gets resized (e.g. an external
 * script wmctl-resizing it), so the bevel always matches the tile's
 * current size. */
void
drawdocktile(Client *c)
{
	...
	XSetForeground(dpy, gc, dockbg.pixel);
	XFillRectangle(dpy, c->win, gc, 0, 0, W, H);

	XSetForeground(dpy, gc, dockhi.pixel);
	XFillRectangle(dpy, c->win, gc, 0, 0, W, 1);
	XFillRectangle(dpy, c->win, gc, 0, 0, 1, H);

	XSetForeground(dpy, gc, docklo.pixel);
	XFillRectangle(dpy, c->win, gc, 0, H - 1, W, 1);
	XFillRectangle(dpy, c->win, gc, W - 1, 0, 1, H);
...
enternotify(XEvent *e)
{
	...
	if (m != selmon) {
		...
		return;
	if (c && c->nofocus) /* hovering a nofocus client (e.g. a dockapp) should never steal focus */
		return;
	focus(c);
}
...
{ /* handling when focus shifts to a different client */
	if (!c || !ISVISIBLE(c)) /* if the target client c isn't visible, */
		for (c = selmon->stack; c && (!ISVISIBLE(c) || c->nofocus); c = c->snext); /* search the stack for the next visible, focusable one */
	...
	if (c) {
		...
		maybefloat(c); /* last-resort catch for a float rule that never matched at manage()-time */
		detachstack(c);
		attachstack(c); /* move c to top of the stack */
		grabbuttons(c, 1);
		XSetWindowBorder(dpy, c->win, scheme[SchemeSel][ColBorder].pixel);
		if (c->twin)
			XSetWindowBorder(dpy, c->twin, scheme[SchemeNotchSel][ColBorder].pixel);
...
{ /* after a window is passed from maprequest, manage determines how to treat it and add it to the stack */
	...
	Window trans = None;
	XWindowChanges wc;
	XWindowAttributes contentwa;
	Window content = None;

	/* DockApp tiles: any window caught by a .isdockapp rule gets wrapped
	 * in our own bezeled background tile (drawdocktile() paints it once
	 * mapped below). Content is the app's XWMHints icon_window if it set
	 * one (real WindowMaker dockapp binaries), otherwise the window
	 * itself (our own PywalDock-toolkit apps, which have no icon_window
	 * and draw straight to their own top-level window). The tile is
	 * auto-sized to content + 2*dockapppad on each side, so non-square
	 * apps still get a snug frame. We copy the original window's
	 * WM_CLASS onto the tile so external tools (wmctl, our own
	 * positioning script) can still find it by class/instance after the
	 * reparent — without this the tile is anonymous and un-findable.
	 * We never touch position/size ourselves beyond this initial wrap;
	 * an external script owns all dock layout. */
	if (matchdockapprule(w)) {
		XWMHints *dwmh = XGetWMHints(dpy, w);
		if (dwmh) {
			if ((dwmh->flags & IconWindowHint) && dwmh->icon_window != None
			&& XGetWindowAttributes(dpy, dwmh->icon_window, &contentwa))
				content = dwmh->icon_window;
			XFree(dwmh);
		}
		if (content == None) {
			content = w;    /* no icon_window: the app's own window is the content */
			contentwa = *wa;
		}
	}
	...
	if (content != None) {
		XClassHint ch = { NULL, NULL };
		Window tile = XCreateSimpleWindow(dpy, root, contentwa.x, contentwa.y,
			contentwa.width + 2 * dockapppad, contentwa.height + 2 * dockapppad, 0, 0, 0);
		if (XGetClassHint(dpy, w, &ch)) {
			XSetClassHint(dpy, tile, &ch);
			if (ch.res_class)
				XFree(ch.res_class);
			if (ch.res_name)
				XFree(ch.res_name);
		}
		XSelectInput(dpy, content, StructureNotifyMask); /* so we hear about it when the app closes */
		XReparentWindow(dpy, content, tile, dockapppad, dockapppad);
		XMapWindow(dpy, content);
		contentwa.width  += 2 * dockapppad;
		contentwa.height += 2 * dockapppad;
		contentwa.border_width = 0;
		w = tile;
		wa = &contentwa;
	}
	...
	{ /* if this window survived a dwm restart, restore whatever tag it was on before */
		...
		if (XGetWindowProperty(dpy, w, tagsatom, 0, 1, False, XA_CARDINAL,
			&type, &format, &nitems, &extra, &data) == Success && data) {
			if (nitems && (*(long *)data & TAGMASK))
				c->tags = *(long *)data & TAGMASK;
			XFree(data);
		}
	}
	...
	if (maximalistmode && !c->isfullscreen && !c->nomaximalist)
		c->isfloating = 1;
		/* ensure window fits to visible bounds of monitor — reserve the
		 * dock column on the right while Maximalist Mode is active, same
		 * as movecorner()/resizemaximalist() do, so new windows don't
	...
	if (c->isfloating && wa->x == 0 && wa->y == 0) {
		c->x = c->mon->wx + (c->mon->ww - rightclear - WIDTH(c)) / 2;
	...
	createnotch(c);
	ensurenotchroom(c); /* spawn/center math above doesn't reserve bar/notch space, so push down if it landed too high */
	updatewindowtype(c);
	updatesizehints(c);
	updatewmhints(c);
...
quit(const Arg *arg)
{
	...
	if (arg->i) { /* restarting, not exiting: stash each client's tag on its own window to survive the re-exec */
		...
		XChangeProperty(dpy, root, viewtagatom, XA_CARDINAL, 32,
			PropModeReplace, (unsigned char *)viewdata, n);
		XSync(dpy, False); /* force these requests out to the X server now — execvp() would otherwise wipe them unsent */
		restart = 1;
	}
	running = 0;
}
...
resizeclient(Client *c, int x, int y, int w, int h)
{
	...
	wc.border_width = c->bw;
	if (c->isshaded)
		wc.x = WIDTH(c) * -2; /* c->x still tracks the real/logical position (so the notch follows correctly), but the actual window stays parked off-screen until unshaded */
...
setup(void)
{
	...
	wa.event_mask = SubstructureRedirectMask|SubstructureNotifyMask
		...
		|LeaveWindowMask|StructureNotifyMask|PropertyChangeMask|KeyPressMask; /* X11 events that dwm will listen for */
...
showhide(Client *c)
{
	...
	if (ISVISIBLE(c)) {
		...
		XMoveWindow(dpy, c->win, c->isshaded ? WIDTH(c) * -2 : c->x, c->y);
		if (c->twin)
			XMoveWindow(dpy, c->twin, c->x, c->y - bh);
		if (!c->isshaded && (!c->mon->lt[c->mon->sellt]->arrange || c->isfloating) && !c->isfullscreen)
...
toggleshade(Client *c)
{
	...
	XMoveWindow(dpy, c->win, c->isshaded ? WIDTH(c) * -2 : c->x, c->y); /* parked off-screen, never unmapped */
	XRaiseWindow(dpy, c->twin); /* stay reachable even if another window would otherwise stack over it */
}
...
togglemaximalist(const Arg *arg)
{
	...
	for (m = mons; m; m = m->next) {
		for (c = m->clients; c; c = c->next) {
			if (c->nomaximalist) /* fully excluded via Rule.nomaximalist */
				continue;
...
notch:
		...
		if (maximalistmode) {
			m->premaxshowbar = m->showbar;
			m->showbar = 0;
		} else {
			m->showbar = m->premaxshowbar;
		}
		updatebarpos(m);
		XMoveResizeWindow(dpy, m->barwin, m->wx, m->by, m->ww, bh);
		arrange(m);
...
void
spawnmaximalist(void)
{
	struct sigaction sa;

	if (maximalistpid > 0)
		return; /* already running */
	if ((maximalistpid = fork()) == 0) {
		if (dpy)
			close(ConnectionNumber(dpy));
		setsid();
		...
		execvp(((char **)maximalistcmd)[0], (char **)maximalistcmd);
		fprintf(stderr, "dwm: execvp %s\n", ((char **)maximalistcmd)[0]);
		perror(" failed");
		_exit(EXIT_FAILURE);
	}
}
...
restoreviewtags(void)
{
	...
	if (XGetWindowProperty(dpy, root, viewtagatom, 0, 32, False, XA_CARDINAL,
		&type, &format, &nitems, &extra, &data) == Success && data) {
		for (m = mons; m && n < nitems; m = m->next, n++) {
			long v = ((long *)data)[n];
			if (v & TAGMASK)
				m->tagset[m->seltags] = v & TAGMASK;
		}
		XFree(data);
	}
	focus(NULL);
